...
from pygame import Rect, constants, draw

from .dynamicrect import DynamicRect
...
class UiWidget:
    def __init__(
        self,
        parent: UiWidget | None = None,
        bg_color: Color | None = None,
        border_color: Color | None = None,
        **kwargs,
    ):
        self.parent_widget: UiWidget | None = None
        self._parent_surface: Surface | None = None
        ...
        self.widgets: list[UiWidget] | None = None
        self.focus_child: UiWidget | None = None
    ...
    def set_parent_surface(self, parent: UiWidget) -> None:
        ...
        self._dyn_rect.set_parent_size_by_surface(
            parent.get_surface(with_borders=False)
        )
        self._parent_surface = None

    def get_root_widget(
        self,
    ) -> UiWidget:  # -> No annotation because root type "unknown/custom"
    ...
    def get_size(self, with_borders: bool = True) -> tuple[int, int]:
    ...
    def set_border(self, border_color: Color | None = None, **kwargs) -> None:
    ...
    def compose(self, surface: Surface) -> bool | None:
        return False

    def compose_borders(self) -> bool | None:
        ...
        if self._dyn_rect.border_top > 0:
            draw.rect(
                ...
                rect=Rect(0, 0, width, self._dyn_rect.border_top),
            )
            drawn = True
        ...
        if self._dyn_rect.border_left > 0:
            draw.rect(
                surface=surface,
                color=self.border_color,
                rect=Rect(0, 0, self._dyn_rect.border_left, height),
            )
            drawn = True
        ...
        if self._dyn_rect.border_right > 0:
            draw.rect(
                ...
                color=self.border_color,
                rect=Rect(
                    width - self._dyn_rect.border_right,
                    0,
                    self._dyn_rect.border_right,
                    height,
                ),
            )
            drawn = True
        ...
        if self._dyn_rect.border_bottom > 0:
            draw.rect(
                ...
                rect=Rect(
                    0,
                    height - self._dyn_rect.border_bottom,
                    width,
                    self._dyn_rect.border_bottom,
                ),
            )
            drawn = True
        return drawn
    ...
    def is_parent_changed(self) -> bool:
        assert self.parent_widget
        parent_changed = (
            self.parent_widget.is_changed() or self.parent_widget.is_parent_changed()
        )
        if parent_changed is True:
            return parent_changed

        assert self.parent_widget.widgets
        ...
        for idx in range(0, own_idx):
            ...
            if under_widget.is_visible is True and under_widget.updated is True:
                if self.get_rect(with_borders=True).colliderect(
                    under_widget.get_rect(with_borders=True)
                ):
                    return True
        return False
    ...
    def set_focus(self, focus: bool = True) -> None:
        ...
        if self.is_focus != focus and self.parent_widget:
            if focus is True:
    ...
    def draw(self) -> None:
        ...
        if self.widgets and (
            self._child_changed is True or self.is_changed() or self.is_parent_changed()
        ):
    ...
    def get_child_by_pos(
        self, pos: tuple[int, int]
    ) -> tuple[UiWidget | None, tuple[int, int] | None]:
        if self.widgets is None:
            return None, None
    ...
    def process_event_focus(self, event: Event) -> bool:
        if (
            self.focus_child is not None
            and self.focus_child.is_visible is True
            and self.focus_child.is_interactive is True
        ):
            return self.focus_child.process_event_focus(event)
        return False

    def process_event_pos(self, event: Event, pos: tuple[int, int]) -> bool:
        pointed_widget, relative_pos = self.get_child_by_pos(pos)
        if pointed_widget and relative_pos:
            # Change focus on mouse button press
            if (
                event.type == constants.MOUSEBUTTONUP
                and event.button <= 3
                and event.touch is False
            ):
